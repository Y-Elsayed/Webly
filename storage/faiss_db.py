...
class FaissDatabase(VectorDatabase):
    ...
    def create(self, dim: int, index_type: str = "flat") -> None:
        ...
        """
        if index_type == "flat":
            self.index = faiss.IndexFlatIP(dim)  # switched to inner product for cosine similarity
        else:
    ...
    def add(self, records: List[Dict]) -> None:
        ...
        arr = self._normalize(arr)  # normalize for cosine similarity
        self.index.add(arr)
    ...
    def search(self, query_embedding: List[float], top_k: int = 5) -> List[Dict]:
        ...
        query = self._normalize(query)  # normalize for cosine similarity
        ...
        for i, idx in enumerate(indices[0]):
            ...
            result["score"] = float(distances[0][i])  # cosine similarity ∈ [-1, 1]
            result["id"] = idx
    ...
    def delete(self, ids: List[int]) -> None:
        ...
        new_arr = np.array(new_embeddings).astype("float32")
        self.index.add(self._normalize(new_arr))
